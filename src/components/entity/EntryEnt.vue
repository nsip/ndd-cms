<template>
    ...
    <div class="area-tabs">
        <button class="scroll-button prev" id="scrollLeftBtn" @click="np_click('<')" :disabled="lHiddenCount == 0" :title="lHiddenCount + ' hidden(s)'">&lt;</button>
        ...
        <button class="scroll-button next" id="scrollRightBtn" @click="np_click('>')" :disabled="rHiddenCount == 0" :title="rHiddenCount + ' hidden(s)'">&gt;</button>
    </div>
...
const updateBothHidden = () => {
    ...
    lHiddenCount.value = h[0].length
    rHiddenCount.value = h[1].length
}

const np_click = async (np: string) => {
    switch (np) {
        case ">":
            if (rHiddenCount.value > 0) {
                startIndex.value++;
            }
            break;
        case "<":
            if (lHiddenCount.value > 0) {
                startIndex.value--;
            }
            break;
    }
